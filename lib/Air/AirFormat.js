...
//TODO filter prices_list by CompleteItinerary=true & ETicketability = Yes, etc
    ...
    async.forEachOf(prices_list, function (price, fare_key) {
        ...
        var passengerCategories = _.mapKeys(price['air:AirPricingInfo'], function(passenger_fare, key) {
            ...
            } else if (_.isObject(code)) { // air:PassengerType in fullCollapseList_obj like above, but there is Age or other info, except Code
                code = code.Code;
                passengerCounts[code] = 1;
            } else if (_.isArray(code)) { // air:PassengerType in noCollapseList uAPI_Parser param
                var count = code.length;
                var list = _.uniq(_.pluck(code, 'Code'));
                if (list.length != 1 || ! (code = list[0])) //TODO throw error
                    console.log("Warning: duplicate categories "+list.join()+" in single fare calculation "+key+ " in fare "+fare_key);
                passengerCounts[code] = count;
            } else
                throw new Error("PassengerTypeCode is supposed to be a string or array"); //TODO
