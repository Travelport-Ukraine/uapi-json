...
var searchLowFaresParse = function(obj){

    var start = new Date();

    if (!obj['air:LowFareSearchRsp'])
        throw new uError('PARSING_AIR_NO_REPLY', obj);

    obj = obj['air:LowFareSearchRsp'][0];
    ...
    root_arrays.forEach(function(name) {
        ...
        if (!obj[air_name] instanceof Array)
            throw new uError('PARSING_AIR_WRONG_TYPE', obj[air_name]);
    });

    var data = this.mergeLeafRecursive(obj, 'air:LowFareSearchRsp');

    var end = new Date() - start;
    console.info("searchLowFaresParse execution time: %dms", end);
    return data;
};
...
var lowFaresSearchRequest = function(obj) {
    return format.formatLowFaresSearch({
        ...
        searchLowFaresParse.bind(this, obj)()
    );
};

var createReservationRequest = function(obj) {
    if (!obj['universal:AirCreateReservationRsp'])
        throw new uError('PARSING_AIR_TICKET_NO_REPLY', obj);

    var data = this.mergeLeafRecursive(obj, 'universal:AirCreateReservationRsp')
...
var importRequestRaw = function (obj) {
    if (!obj['universal:UniversalRecordImportRsp'])
        throw new uError('PARSING_AIR_IMPORT_NO_REPLY', obj);

    return this.mergeLeafRecursive(obj, 'universal:UniversalRecordImportRsp')
        ['universal:UniversalRecordImportRsp']; //FIXME careful!
};
...
var importRequest = function(obj) {
    var data = importRequestRaw.bind(this, obj)();
...
var AirPriceFareRules = function(obj) {
    ...
    //if (!obj['air:AirPriceRsp'])
    //    throw new uError('PARSING_AIR_PRICING_NO_REPLY', obj);
...
var FareRules = function(obj) {
    ...
    var data = _.mergeLeafRecursive(obj, 'air:AirFareRulesRsp')['air:AirFareRulesRsp'];

    return extractFareRulesLong(data);
};
...
var SearchErrorHandler = function(obj) {
    //TODO check root object

    var result = this.mergeLeafRecursive(obj, 'SOAP:Fault')['SOAP:Fault'];
    return result;
};
...
var gdsQueue = function (req) {
    if (req['gdsQueue:GdsQueuePlaceRsp']
        && req['gdsQueue:GdsQueuePlaceRsp'][0]
        && req['gdsQueue:GdsQueuePlaceRsp'][0]['common_v36_0:ResponseMessage']) {
        var message = req['gdsQueue:GdsQueuePlaceRsp'][0]['common_v36_0:ResponseMessage'][0]['_'];
        if (message.match(/^Booking successfully placed/) === null) {
            throw new uError('GDS_PLACE_QUEUE_ERROR', req);
        }
    } else {
        throw new uError('GDS_PLACE_QUEUE_ERROR', req);
    }
