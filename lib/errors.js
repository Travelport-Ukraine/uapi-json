var allErrors = {
    ...
    VALIDATION_PEOPLE : {errno: 110, msg: "Missing people is request. Must be more that 1 traveler."},
    ...
    PARSING_HOTELS_SEARCH_ERROR: {errno: 151, msg: "Cant parse XML repsonse. #HotelsParser.searchParse()"},
    PARSING_HOTELS_MEDIA_ERROR: {errno: 152, msg: "Cant parse XML repsonse. #HotelsParser.kediaParse()"},
    PARSING_HOTELS_RATES_ERROR: {errno: 153, msg: "Cant parse XML repsonse. #HotelsParser.rateParse()"},
    PARSING_HOTELS_BOOKING_ERROR: {errno: 154, msg: "Cant parse XML repsonse. #HotelsParser.bookParse()"},
    PARSING_HOTELS_CANCEL_BOOKING_ERROR: {errno: 155, msg: "Cant parse XML repsonse. #HotelsParser.cancelBookParse()"},

    // AIR validation
    PASSENGERS_REQUIRED_LOW: {errno: 1000, msg: "Passengers hash of passenger types is required for #Air.searchLowFares()"},
    PASSENGERS_CATEGORY_INVALID: {errno: 1001, msg: "Passengers hash includes an invalid passenger category or requested quantity is not a number in #Air.searchLowFares()"},
    LEGS_REQUIRED: {errno: 1002, msg: "Legs should be an array in request for #Air.searchLowFares()"},
    LEGS_REQUIRED_STRUCTURE: {errno: 1003, msg: "Leg in leg array does not have all required fields for #Air.searchLowFares()"},
    ...
    PARSING_AIR_TICKET_NO_REPLY: {errno: 1300, msg: "No air:AirTicketingRsp object in parsed XML for #Air.ticket()"},
...
module.exports = function(type){
    ...
    if (details){
        var error = new Error(allErrors[type].msg + ' Details provided.');
        ...
        var error = new Error(allErrors[type].msg);
