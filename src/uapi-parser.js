...
const getItemEmbKey = function (item, key) { // TODO: for branded fares, air:Brand is referred by BrandID or BrandFound=false and keys are useless
  if (item['$'] && item['$'].Key)
    return item['$'].Key;
  else return false;
};

const mergeLeaf = function (item) {
  const leaf = item['$'];
  delete (item['$']);
...
const defaultConfig = function (ver) {
  ...
  const fullCollapseSingleKeyed_obj = [
  ...
  return {
    noCollapseList,
    fullCollapseList_obj,
    fullCollapseSingleKeyed_obj,
    CollapseKeysOnly,
    dropKeys,
  };
};
...
const Parser = function (root, uapi_version, env, config) {
  if (!config)
...
Parser.prototype.mergeLeafRecursive = function (obj, name) {
  ...
  if (_.isArray(obj)) {
    let list_name = (name.substr(-4) == 'List') ? name.substring(0, name.length - 4) : name;
...
Parser.prototype.parseXML = function (xml) {
  return parseString(xml).then(function (res) {
    if (res['SOAP:Envelope']) {
      return responseJSON = res['SOAP:Envelope']['SOAP:Body'][0];
    } else {
      throw new uError('PARSING_ERROR', res); // TODO replace with custom error, parse other SOAP errors?
    }
  }, function (err) {
...
Parser.prototype.parse = function (xml) {
  ...
  return this.parseXML(xml).then(
        function (obj) {
          const start = new Date();

          if (!self.rootObject)
            return obj; // fall back to custom non-uAPI parser interface with dirty code //FIXME rewrite Hotels
